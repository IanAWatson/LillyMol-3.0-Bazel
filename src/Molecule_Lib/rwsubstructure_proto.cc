                                           ...
                                           extending_resizable_array<Substructure_Atom *> & completed)
{
  ...
  for (const auto& substructure_atom : proto.substructure_atom())
  {
    ...
    {
      cerr << "_create_environment_from_proto: cannot create environment component " << substructure_atom.DebugString() << endl;
      delete a;
      return 0;
    }
  }
...
//#define DEBUG_COMPLTED
  ...
  if (!proto.has_id())
  {
    cerr << "Substructure_Atom::_construct_from_proto:no id " << proto.DebugString() << "\n";
    return 0;
  }
...
// Process any bonds which have been specified as attributes
  ...
  {
    cerr << "Substructure_Environment::construct_from_proto:cannot connect " << proto.DebugString() << "\n";
    return 0;
  }
...
//  cerr << "Build query from smiles " << (*att) << endl;
  ...
  for (const auto& query_atom: proto.query_atom()) {
    ...
    else
    {
      ...
      cerr << query_atom.DebugString() << endl;
      delete a;
      return 0;
    }
  }
